...
import { isManual, isRedirectProvider, isStripeLike } from "@lib/constants"
...
function captureMetaSignals(cartId: string) {
  try {
    const fbp = document.cookie.match(/_fbp=([^;]+)/)?.[1]
    const fbc = document.cookie.match(/_fbc=([^;]+)/)?.[1]
    if (fbp || fbc) {
      const meta: Record<string, string> = {}
      if (fbp) meta.meta_fbp = fbp
      if (fbc) meta.meta_fbc = fbc
      updateCartMetadata(cartId, meta).catch(() => {})
    }
  } catch {
    // non-blocking
  }
