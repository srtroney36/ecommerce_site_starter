...
function ErrorRow({ err }: { err: ClientError }) {
  ...
  return (
    <div className="border border-ui-border-base rounded-lg overflow-hidden">
      ...
      >
        <span
          className="text-ui-fg-muted mt-0.5 transition-transform"
        ...
        <div className="border-t border-ui-border-base bg-ui-bg-subtle px-4 py-3 flex flex-col gap-y-3">
          ...
          {err.stack && (
            <div className="flex flex-col gap-y-1">
              ...
              <pre className="text-[11px] leading-relaxed text-ui-fg-subtle bg-ui-bg-base border border-ui-border-base rounded-md p-3 overflow-x-auto whitespace-pre-wrap break-words max-h-72">
                {err.stack}
              </pre>
            </div>
          )}
    ...
    <div className="flex flex-col gap-y-4 p-4">
      <Container className="px-6 py-6 flex flex-col gap-y-5">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <Heading level="h1">Error Log</Heading>
            <Text size="small" className="text-ui-fg-subtle mt-1">
              Errors customers hit on the storefront. Click a row to expand the full details.
            </Text>
          </div>
          <div className="flex items-center gap-2">
            <Button size="small" variant="secondary" disabled={loading || busy} onClick={load}>
              Refresh
            </Button>
            {errors.length > 0 && (
              <Button
      ...
      <Prompt open={confirmClear} onOpenChange={setConfirmClear}>
        <Prompt.Content>
          <Prompt.Header>
            ...
            <Prompt.Description>
              This permanently deletes all recorded errors. This cannot be undone.
            </Prompt.Description>
          </Prompt.Header>
          <Prompt.Footer>
